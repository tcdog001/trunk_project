...
get_GPS_Satellite() {
	...
	GPSGPGGA=`grep -r $GPSSatellite /tmp/gps_info |sed -n '$p'`
	if [ -z "$GPSSatellite" ];then
		GPSSatellite="00"
	fi
	echo "$GPSSatellite"
	echo "$GPSGPGGA" >/tmp/GPSGPGGA
}

get_GPS_Location() {
	GPSLocation=`cat /tmp/GPSGPGGA |awk -F ',' '{print $4$3$6$5}'`
...
get_GPS_Height() {
	GPSHeight=`cat /tmp/GPSGPGGA |awk -F ',' '{print $10}'`
	if [ -z "$GPSHeight" ];then
...
get_GPS_Time() {
	GPSTime=`cat /tmp/GPSGPGGA |awk -F ',' '{print $2}'`
	if [ -z "$GPSTime" ];then
...
get_3G_Connection() {
	Connection3G=`ifconfig |grep 3g-evdo 2>/dev/null`
