...
#define hlist_get(pos, head, member, data, eq)  ({ \
    typeof(*(pos)) *node = NULL;                    \
                                                    \
...
#ifndef MLISTSIZE
#define MLISTSIZE       256
#endif

#ifndef MLISTMASK
#define MLISTMASK       (MLISTSIZE-1)
#endif
...
struct mlist_head {
    struct list_head    list;
    struct hlist_head   hash[MLISTSIZE];
};
...
static inline bool
is_good_mlist_idx(int idx)
{
    return is_good_enum(idx, MLISTSIZE);
}

static inline bool
is_in_mlist(struct mlist_node *node)
{
    return is_in_list(&node->list);
}

static inline int
mlist_string_hash_idx(char *string)
{
    return __string_hash_idx(string, MLISTSIZE);
}
...
)
{
    ...
    */
    ...
    int idx = (*hash)();
    if (false==is_good_mlist_idx(idx)) {
        return -EBADIDX;
    }
...
static inline int
mlist_remove(struct mlist_head *head, struct mlist_node *node)
{
    if (NULL==node) {
        return -EINVAL9;
    }
    ...
    * NOT in list
    */
    else if (false==is_in_list(&node->list)) {
        return -ENOINLIST;
    }
...
)
{
    ...
    return hlist_get(node, &head->hash[idx], hash, data, eq);
}
