...
#ifndef INLINE_COROUTINE
#define INLINE_COROUTINE        __CO
#endif

#define USE_INLINE_COROUTINE    co_control_t INLINE_COROUTINE
extern USE_INLINE_COROUTINE;

static inline co_control_t *
__co(void)
{
#ifdef __USE_INLINE_COROUTINE
    return &INLINE_COROUTINE;
#else
    return NULL;
#endif
}
...
co_name(co_id_t id)
{
    ...
    if (false==__is_good_coid(id)) {
        return os_assert_value(CO_NOEXIST_NAME);
    }
    else if (false==__is_good_co_running()) {
        return os_assert_value(CO_NOEXIST_NAME);
    }
